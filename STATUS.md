# Project Status
### Last updated: 2026-07-01 (latest) — **Birth Story REGISTER shift + Brief rebuilt as the template.** Layout-polish pass shipped first (Brief/Brand/Outcome balance, motion +15%), then a body-copy tightening pass, then a **register redirect** per Lorin: the case study moved from narrated prose to **structured documentation in three legible voices** — (1) objective record = direct sentences + `Required`/`Provided` bullet lists, (2) her thinking = a new teal **`<Insight>` "My thinking" callout** (mirror of the terracotta tester note), (3) tester feedback = the existing crit voice. **Hero, Overview, and the Brief are done in this register** (commits `6fc82fd`, `4397c9b`). Full rationale: memory `DECISIONS.md → Case-study REGISTER`. Client/asset facts: `REFERENCE.md → Birth Story client facts`. **Next: apply the register to Research → Close.**
...
**Next move — APPLY THE REGISTER to Research → Close (resume here).** Hero + Overview + Brief are done in the three-voice register (commits `6fc82fd`, `4397c9b`); the remaining sections (Research, Architecture, Iteration, Features, Voice, Brand, Outcome, Close) are still on the OLD autobiographical-prose register and need converting: objective content → direct + bulleted; her reasoning → concrete `<Insight>` callouts drafted from her real notes (mark `[LORIN TO WRITE]` where unsourceable). Lorin asked to bring them back a couple of sections at a time. **The Brief is the locked template — match it.**
- **Open layout Q on the Brief:** the bottom band's short "My connection" sits beside a tall photo (gap under the text). Lorin to decide: shrink the photo, or leave it (photo-dominant).
- **Personal-image idea:** Lorin floated a real photo of her with a baby for the Brief context slot (currently stock B&W). Recommendation logged: keep the composed stock unless she has a genuinely composed image.
- **Older open copy slots (still on page):** the `[LORIN TO WRITE]` rename of "Find strength & support" in UX Writing.
