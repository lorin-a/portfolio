...
**Next move — THE COPY PASS (resume here):** The whole case study is rebuilt and uniform; the remaining work is **real copy in Lorin's voice**. Lorin's full interview (Q1–Q16) + the official brief/requirements are captured in `docs/case-study-editorial/BIRTHSTORY-interview.md`; the **build plan, spine drafts, standing corrections, and resume point** are in `docs/case-study-editorial/BIRTHSTORY-copy-plan.md` (START THERE). Raw source preserved in `BIRTHSTORY-brief-source.txt` + `BIRTHSTORY-requirements-source.txt`.
- **Pending Lorin decisions (block the build):** (1) confirm kept/cut feature map; (2) Brief tone — systemic maternal-mortality stats for gravity vs. keep warm/personal; (3) read `MY PART.docx` (LaCie) for contribution language; (4) source/soften the "birth ≈ Olympic endurance" stat.
- **Must-fix on build:** Outcome overclaims "intend to build" → truth is *client sponsored it, may inform future Myana, no commitment*. Make "designed + built by me (with AI)" explicit; app is a *concept*, nothing shipped.
- **Also queued (Lorin's layout notes, in PROGRESS.md):** Voice reads draft-y/empty; Brand → compact right-side view, kill moodboard whitespace; Outcome photo overlaps text, follow page-wide grid; animations +10–20%.
- Then: apply the register to **Transition Design** (star #3); then the **"More work" / All Work bento + Playground**.
