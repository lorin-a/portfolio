# Project Status
### Last updated: 2026-06-30 (latest) — **Workflow/tooling session — no project code changed.** Added a lean **Intake rule** + an **engineering-authority / creative split** to global CLAUDE.md, locked an **over-tooling audit discipline** (build only what a cited failure demands), granted **commit-freely** permission, and built a **permission allowlist** (safe MCP reads auto-allowed; `rm` / `browser_evaluate` / click-type kept gated on purpose). Rationale in memory `DECISIONS.md`; full research in `docs/UPSKILL_RESEARCH_2026.md`. **Project state is unchanged — Next move is still the Birth Story layout-polish pass (below).**
### Prev (2026-06-30) — Birth Story COPY PASS shipped live. Rebuilt from question-led "field notes" into hiring-manager documentation: Overview masthead + declarative `<Lead>` + autobiographical first-person `<Prose>` (question structure retired per Lorin). Type fix: body prose off Fraunces → Open Sans (Fraunces reserved for leads/headings/callouts). Overclaim removed; voice-audit passed. **Source of truth for copy = `components/Birthstory/*`, not the copy doc. Next: layout-polish pass.**
...
**The locked direction — process documentation, organized like a hiring manager, narrated autobiographically** (full principle in memory `DECISIONS.md → "Case-study VOICE"`, UPDATED 2026-06-30): the earlier question-led "field notes" register was **CUT** (Lorin: "forget your question structure"). Now an **Overview masthead** (role/team/methods/outcome + synopsis) leads, then each section opens on a **declarative `<Lead>`** and continues in **autobiographical first-person `<Prose>`** — her narrating her own process, never pitch / news-anchor / poetic / quippy. **Type:** body prose is the body font (Open Sans); **Fraunces is reserved for leads, headings, callouts.** Her real Figma screens as figures (NOT recreations); cool paper ground `#F1F2F0`. Candidate model for Transition Design + the other stars. **Copy source of truth = `components/Birthstory/*` (the page), not the `BIRTHSTORY-copy.md` doc.**
...
**Next move — LAYOUT-POLISH PASS (resume here):** copy is settled; the open work is layout. From Lorin's notes (also `PROGRESS.md`): **Brief whitespace** (short text column leaves a tall gap beside the stacked photos, sub-blocks sit below) · **Brand** → compact right-side view, kill moodboard whitespace · **Outcome** photo/text alignment to the page grid · **animations +10–20%**.
- **2 copy slots still open (Lorin's call, visible on page):** the `[LORIN TO WRITE]` rename of "Find strength & support" in UX Writing; the optional systemic-stats line in the Brief (currently cut).
- Then: apply this register to **Transition Design** (star #3; source = `MY PART.docx` Tab 2, see memory `REFERENCE.md`); then the **All Work bento + Playground**.
