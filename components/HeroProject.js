...
export default function HeroProject({ project }) {
  ...
  const content = (
    <>
      {/* Full-Width Image */}
      <div className={`${styles.imageContainer} ${isHovered && !isComingSoon ? styles.hovered : ''}`}>
        {project.hero ? (
          <img
      ...
      <div className={styles.info}>
        <h2 className={`${styles.title} ${isHovered && !isComingSoon ? styles.titleHovered : ''}`}>
          {project.title}
        </h2>
  ...
  return (
    <article
      className={`${styles.project} ${isComingSoon ? styles.comingSoon : ''}`}
