...
> **IMPLEMENTED LIVE 2026-06-30, then revised twice past this doc.** The page at `/projects/birthstory-care-pod` (port 3001) is now the source of truth for copy, not this doc. After v3 shipped, Lorin's feedback drove two more passes ON THE PAGE: (1) **autobiographical voice** — body prose rewritten into flowing first-person narration (no news-anchor declaratives, no quippy openers/closers); (2) **type hierarchy** — `.prose` moved off Fraunces onto the body font so Fraunces is reserved for leads/headings/callouts; (3) a **voice-audit pass** (contractions, killed "hold space," curly quotes). The section prose in this doc below is now SUPERSEDED by the components; read the page for current copy. Still open: the `[LORIN TO WRITE]` rename in UX Writing (visible placeholder on the page) and the `[OPTIONAL]` systemic line in the Brief (currently cut).
