...
/** Cloudinary image URL with auto-format, auto-quality, optional width, an
 *  optional grayscale effect (`{ grayscale: true }`), optional extra
 *  effects (`{ effects: ['improve:40', 'brightness:10'] }` → `e_*` chain),
 *  and an optional raw transformation chain (`{ chain: [...] }`) — slash-
 *  separated URL components applied before delivery, as produced by
 *  Cloudinary's Media Editor (each entry is one component, applied in order). */
export function cloudImg(publicId, width, opts = {}) {
  const t = ['f_auto', 'q_auto']
  if (opts.grayscale) t.push('e_grayscale')
  if (opts.effects) opts.effects.forEach((e) => t.push(`e_${e}`))
  if (width) t.push(`w_${width}`)
  const pre = opts.chain ? `${opts.chain.join('/')}/` : ''
  return `${BASE}/image/upload/${pre}${t.join(',')}/${publicId}`
}
...
export const HOME_IMAGES = {
  ...
  'td-stakeholders-full':   'Team_Autopoesis_Stakeholder_Relations',
  'td-timeline-full':       'Team_Autopoesis_FoodInsecurity_Timeline_2025',
  'td-roots':               'Team_Autopoesis_Roots_to_Resilience',
  'td-4b':                  'Team_Autopoesis_4B',
  ...
  'btg-36':                 'BTG_36',
  'btg-45':                 'BTG_45',
  'btg-early-prototype':    'Early_App_Prototype_GIF',
...
export const GS_IMAGES = {
  ...
  'gs-mockups-43':              'Groundswell_Mockups_Lorin-43',
  'gs-mockups-50':              'Groundswell_Mockups_Lorin-50',
  'groundswell-ctb-docs':       'groundswell-ctb-docs',
  'gs-ctb-email':               'gs-ctb-email',
  'gs-pod':                     'gs-pod',
  'gs-pod-detail-01':           'gs-pod-detail-01',
  'gs-pod-detail-02':           'gs-pod-detail-02_sshu8f',
  'gs-pod-detail-03':           'gs-pod-detail-03_sfmbb6',
...
export const HOME_VIDEOS = {
  ...
  'somebuddy-animation':         'Animation',
  'somebuddy-animation-2':       'Animation_2',
  'somebuddy-animation-3':       'Animation_3',
  'somebuddy-ux':                'UX',
  'somebuddy-ux-2':              'UX_2',
  ...
  'bridging-clip-1':             'BTG-Clip-1',
  'bridging-clip-2':             'BTG-Clip-2',
  'bridging-logos-2':            'Logos_2',
  'bridging-logos-3':            'Logos_3',
  'bridging-gap-logo':           'GAP_logo_Animated',
}
...
export const WHELM_IMAGES = {
  ...
  'whelm-3':      '3',
  'whelm-4':      '4',
  'whelm-slide-8':  'Slide_16_9_-_8',
  'whelm-slide-9':  'Slide_16_9_-_9',
  'whelm-slide-11': 'Slide_16_9_-_11',
  'whelm-slide-13': 'Slide_16_9_-_13',
  'whelm-slide-17': 'Slide_16_9_-_17',
  'whelm-slide-22': 'Slide_16_9_-_22',
  'whelm-slide-27': 'Slide_16_9_-_27',
}
...
export const WHELM_VIDEOS = {
  ...
  'whelm-preview': 'whelm_preview_5.5',
}

// ── Birth Story — licensed Pexels context photos (the lived birth experience).
//    Each maps to a case-study moment. `by` is the photographer credit pulled
//    from the Pexels filename; rendered as "Photo · {by} / Pexels". `o` is the
//    natural orientation (so layouts can size the slot without a forced crop). ──
export const BIRTHSTORY_PHOTOS = {
  'fog':        { id: 'pexels-saul-siguenza-355267816-36288592', by: 'Saul Siguenza',       o: 'landscape' }, // exhausted parent + swaddled newborn, teal
  'reflect':    { id: 'pexels-william-fortunato-6392913',        by: 'William Fortunato',    o: 'portrait'  }, // mother gazing at sleeping baby, at home
  'room':       { id: 'pexels-jonathanborba-3279209',            by: 'Jonathan Borba',       o: 'landscape' }, // partner holds newborn, sibling leans in
  'circle':     { id: 'pexels-greyartweddings-29172142',         by: 'Grey Art Weddings',    o: 'portrait'  }, // two parents' hands cradle a tiny foot, b&w
  'disorient':  { id: 'pexels-by_narmin-aslanli-701641061-29127872', by: 'Narmin Aslanli',   o: 'landscape' }, // hand holding a newborn foot, hospital, b&w
  'tender':     { id: 'pexels-sarah-chai-7282807',               by: 'Sarah Chai',           o: 'portrait'  }, // mother rests cheek to cheek with her sleeping newborn, b&w
  'gap':        { id: 'pexels-william-fortunato-6392902',        by: 'William Fortunato',    o: 'landscape' }, // mother props her head, gazing tired at her sleeping newborn beside her
  'feeding':    { id: 'pexels-thatguycraig000-4041805',          by: 'Craig Adderley',       o: 'landscape' }, // mother holds newborn skin-to-skin in a hospital bed (Brief context, used B&W)
  'cradle':     { id: 'pexels-thatguycraig000-4041804',          by: 'Craig Adderley',       o: 'landscape' }, // same mother cradles + kisses her swaddled newborn, IV pole (alt Brief context)
  'birth':      { id: 'pexels-jonathanborba-3259628',            by: 'Jonathan Borba',       o: 'landscape' }, // a newborn lifted over the mother just after delivery (alt Brief context)
}

/** Birth Story context photo → { src, byline, orientation }. Pass
 *  `{ grayscale: true }` to render it black and white. */
export function birthPhoto(slug, width = 900, opts) {
  const p = BIRTHSTORY_PHOTOS[slug]
  if (!p) return null
  return { src: cloudImg(p.id, width, opts), byline: p.by, orientation: p.o }
}
